(function () {
  ...
  onKey("escape", (e) => {
    ...
    // first so the grid restores scroll position on arrival. preventDefault
    // owns the press: without it Chrome's Esc default (stop loading) races
    // the navigation while the full-res photo is still in flight, eating the
    // first press — the global up-nav and bindEscapeToClear both do the same.
    e.preventDefault();
