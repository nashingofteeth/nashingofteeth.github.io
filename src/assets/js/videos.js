...
for (const link of links) {
  link.addEventListener("click", function (e) {
    ...
    if (youtubeId) {
      ...
      iframe.setAttribute("title", mediaTitle(true));
      ...
      video.setAttribute("title", mediaTitle(true));
...
// ---------------------------------------------------------------------------
function mediaTitle(initialized) {
  // Single pages advertise Enter only on the uninitialized cover (it starts
  // playback); once the player exists, Space/F cover it.
  if (isVideoSinglePage()) {
    return initialized ? "Play (Space/F)" : "Play (Enter/Space/F)";
  }
  return "Play (Space/F)";
}
...
function refreshVideoHints() {
  ...
  if (article) {
    ...
    const playLink = article.querySelector(".container .media a[href]");
    const player = article.querySelector("video, iframe[data-yt-player]");
    const media = playLink || player;
    if (media) {
      // A surviving play link means uninitialized; a player means initialized.
      next.push([media, mediaTitle(!playLink)]);
    }
